     ...
     (eval
      (lambda nil
        (when
          ...
          (let
              ((shimmers-file
                (expand-file-name "shimmers.el" default-directory)))
            (when
                (file-exists-p shimmers-file)
              (load shimmers-file)
              (require 'shimmers))))))
