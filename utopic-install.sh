...
sudo apt install emacs-snapshot emacs-snapshot-el vim \
     fonts-inconsolata fonts-dejavu fonts-cascadia-code fonts-roboto \
...
sudo apt install rlwrap shellcheck graphviz \
     ...
     default-jdk openjdk-8-jdk openjdk-17-jdk openjdk-21-jdk openjdk-25-jdk\
...
sudo apt install --yes nodejs yarn
...
(export CHROME=https://storage.googleapis.com/chrome-for-testing-public/148.0.7778.5/linux64/chromedriver-linux64.zip &&
...
curl --output clj.sh -O https://download.clojure.org/install/linux-install-1.12.3.1577.sh &&
